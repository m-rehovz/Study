...
def original_function(t):
    return (np.sin(t) + 0.5*np.cos(2*t) + 0.2*np.sin(3*t) +
            0.1*np.cos(5*t) + 0.05*np.sin(7*t) +
            0.03*np.cos(9*t) + 0.02*np.sin(10*t)*np.cos(3*t))


def compute_fourier_coefficients(N):
    results = {'a': [], 'b': [], 'c': []}

    # a0 (должен быть 0 для нечётной функции)

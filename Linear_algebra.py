...
def multiplication(tuple_of_matrices):        # умножение
    ...
    m2 = tuple_of_matrices[1]
    if len(m1) != len(m2[0]):
        print('Ой ой, так нельзя (есть правило для умножения матриц разного размера)')
    else:
    ...
    def check_basis(basis):
        for i in range(len(basis)):
            if len(basis[i]) != len(basis):
                return 'лимонная долька такая же кислая как твоя рожа (вы что-то делаете не так)'
...
def inputVectors():  # функция для корректного ввода векторов
    ...
    for i in range(n):
         try:
             ...
             raise Exception('Пожалуйста, введите элементы вектора через пробел! ')
    return basis_list
